...
; clear all relevant hardware registers & wram
	...
	ld a, AUDENA_ON  ; all channels on
...
;   sound channel   1      2      3      4
	xor a
	ld [hli], a ; rAUD1SWEEP, AUD2LEN -1, rAUD3ENA, AUD4LEN -1 ; sweep = 0
	...
	ld a, $80   ; AUD1HIGH_RESTART, AUD2HIGH_RESTART, AUD3HIGH_RESTART, AUD4GO_RESTART
...
.ch1_check_duty_override
	...
	ld d, a
	ldh a, [rAUD1LEN]
	and AUD1LEN_TIMER ; sound length
	or d
	ldh [rAUD1LEN], a
	ret
...
.ch1_noise_sampling
	ld hl, wCurTrackDuty
	ld a, AUD1LEN_TIMER ; sound length
	or [hl]
	ldh [rAUD1LEN], a
...
.ch2_check_duty_override
	...
	ld d, a
	ldh a, [rAUD2LEN]
	and AUD2LEN_TIMER ; sound length
	or d
	ldh [rAUD2LEN], a
	ret
...
.ch2_noise_sampling
	ld hl, wCurTrackDuty
	ld a, AUD2LEN_TIMER ; sound length
	or [hl]
	ldh [rAUD2LEN], a
	ld a, [wCurTrackVolumeEnvelope]
	ldh [rAUD2ENV], a
	...
	ldh [rAUD2LOW], a
	ld a, [wCurTrackFrequency + 1]
	or AUD2HIGH_RESTART ; initial (restart)
	ldh [rAUD2HIGH], a
	ret
...
endr
	...
	cp $f
	jr z, .skip
	; load wavepattern into rAUD3WAVE_0 equ - rAUD3WAVE_F
	ld a, [hli]
	ldh [rAUD3WAVE_0], a
	ld a, [hli]
	ldh [rAUD3WAVE_1], a
...
.ch4_noise_sampling
	ld a, AUD4LEN_TIMER ; sound length
	ldh [rAUD4LEN], a
	ld a, [wCurTrackVolumeEnvelope]
	ldh [rAUD4ENV], a
	ld a, [wCurTrackFrequency]
...
.ch7
	...
	ld a, $8 ; unused bits in rAUD3LEVEL — likely harmless, but unclear
...
ClearChannel:
; input: hl = beginning hw sound register (rAUD1SWEEP, AUD2LEN -1, rAUD3ENA, AUD4LEN -1)
